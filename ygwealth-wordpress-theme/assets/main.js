...
const wpTheme = window.ygTheme || {};
const wpRoutes = wpTheme.routes || {};
...
function cleanUrl(path) {
  ...
  const cleaned = path.replace(/\.html(?=($|[?#]))/, "");
  if (/^(https?:|mailto:|tel:|#)/.test(cleaned)) return cleaned;

  const suffixMatch = cleaned.match(/[?#].*$/);
  const suffix = suffixMatch ? suffixMatch[0] : "";
  const pageKey = cleaned.replace(/[?#].*$/, "").replace(/^\/+/, "").replace(/\/$/, "") || "index";

  if (wpRoutes[pageKey]) {
    return `${wpRoutes[pageKey]}${suffix}`;
  }
...
function applyPageHeroImage(type, item) {
  ...
  if (!hero) return;
  const page = location.pathname.replace(/\/$/, "").split("/").pop().replace(".html", "") || "index";
  ...
  header.innerHTML = `
    <nav class="nav" aria-label="Main navigation">
      <a class="logo" href="index.html" aria-label="Y&G Financial Services home">
        <img class="logo__mark" src="${assetUrl("yg-logo.png")}" alt="Y&G Financial Services">
      </a>
  ...
  footer.innerHTML = `
    <div class="footer-inner">
      <div class="footer-grid">
        <div>
          <img class="footer-brand" src="${assetUrl("yg-logo.png")}" alt="Y&G Financial Services">
          ...
          <div class="membership-badges" aria-label="Membership details">
            <div class="membership-badge">
              <img class="membership-img" src="${assetUrl("amfi-logo.svg")}" alt="AMFI logo">
              <div><strong>AMFI Registered</strong><small>Mutual Fund Distributor · ARN-134795</small></div>
            </div>
            <div class="membership-badge">
              <img class="membership-img" src="${assetUrl("bse-starmf-logo.svg")}" alt="BSE StAR MF logo">
              <div><strong>BSE StAR MF</strong><small>Platform membership for mutual fund transactions</small></div>
            </div>
            <div class="membership-badge">
              <img class="membership-img" src="${assetUrl("nse-nmf-logo.svg")}" alt="NSE NMF logo">
              <div><strong>NSE NMF</strong><small>Mutual fund transaction platform</small></div>
            </div>
            <div class="membership-badge">
              <img class="membership-img" src="${assetUrl("mfu-logo.svg")}" alt="MFU logo">
